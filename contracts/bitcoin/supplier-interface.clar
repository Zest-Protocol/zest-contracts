...
(define-public (finalize
  ...
  (let (
      ...
      (try! (contract-call? .pool-v1-0 send-funds lp-token token-id zp-token (get sats swap) factor height lv contract-caller xbtc-ft))
...
(define-public (send-funds-wrap
  ...
  (let (
    ...
    (let (
      ...
      (try! (contract-call? .pool-v1-0 send-funds lp-token token-id zp-token (get sats swap) factor (get height block) lv contract-caller xbtc-ft))
      (ok (get sats swap))
    )
  )
)
...
(define-public (make-payment-wrap
  ...
  (let (
    ...
    (swapper (get swapper-principal swap))
      ...
      (ok (try! (contract-call? .loan-v1-0 make-payment loan-id height payment lp-token token-id cp-token zp-token swap-router (get sats swap) contract-caller xbtc-ft)))
    )
  )
)
...
(define-public (withdraw
  ...
  (xbtc-ft <ft>)
    ...
    (begin
      (try! (contract-call? .pool-v1-0 withdraw lp-token token-id lv xbtc contract-caller xbtc-ft))
...
(define-public (drawdown
  ...
  (let (
    (result (try! (contract-call? .pool-v1-0 drawdown loan-id lp-token token-id coll-token coll-vault fv contract-caller xbtc-ft)))
    (swap-id (try! (as-contract (contract-call? .bridge initiate-outbound-swap (get borrow-amount result) btc-version btc-hash supplier-id))))
  )
    (try! (contract-call? coll-token transfer (get coll-amount result) contract-caller (as-contract tx-sender) none))
    (try! (contract-call? coll-vault store coll-token (get coll-amount result) loan-id))

    (print { btc-version: btc-version, btc-hash: btc-hash, supplier-id: supplier-id })
    (ok { swap-id: swap-id, sats: (get borrow-amount result) })
  )
)
