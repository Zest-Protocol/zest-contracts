...
(define-read-only (get-balance (account principal))
  (ok (ft-get-balance zest-dist account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? zest-dist amount sender recipient)
      response (let (
...
(define-public (withdraw-funds)
  (let (
    ...
    (try! (as-contract (contract-call? .zge000-governance-token transfer withdrawable-funds tx-sender recipient none)))
...
(define-private (mint-priv (recipient principal) (amount uint))
  (begin
    ...
    (ft-mint? zest-dist amount recipient)
  )
)
...
(define-private (burn-priv (owner principal) (amount uint))
  (begin
    ...
    (ft-burn? zest-dist amount owner)
  )
)
...
(define-read-only (accumulative-funds-of (owner principal))
  (/
    (to-uint (+
        (to-int (* (var-get points-per-share) (ft-get-balance zest-dist owner)))
        (default-to 0 (map-get? points-correction owner))
    ))
    POINTS_MULTIPLIER
  )
...
(define-private (update-funds-balance (delta int))
  (begin
    ...
    (asserts! (and (> delta 0 ) (> (ft-get-supply zest-dist) u0)) delta)
    ;; only distribute when funds are earned
    (var-set points-per-share (+ (var-get points-per-share) (/ (* (to-uint delta) POINTS_MULTIPLIER) (ft-get-supply zest-dist))))
    delta
  )
)
