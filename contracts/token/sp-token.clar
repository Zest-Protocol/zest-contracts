...
(define-read-only (get-total-supply) (ok (ft-get-supply sp-001)))

(define-read-only (get-name) (ok "Staking Pool"))

(define-read-only (get-symbol) (ok "Staking Pool Token"))

(define-read-only (get-decimals) (ok u0))

(define-read-only (get-balance (account principal))
  (ok (ft-get-balance sp-001 account))
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? sp-001 amount sender recipient)
      response (let (
...
(define-public (withdraw-funds)
  (let (
    ...
    (staker-data (try! (contract-call? .staking-pool-v1-0 get-staker-data tx-sender)))
    )
    (try! (as-contract (contract-call? .zge000-governance-token transfer withdrawable-funds FUNDS_HOLDER_CONTRACT recipient none)))
    (try! (contract-call? .rewards-multiplier mint-rewards tx-sender withdrawable-funds (get cycles staker-data)))
...
(define-private (mint-priv (recipient principal) (amount uint))
  (begin
    ...
    (ft-mint? sp-001 amount recipient)
  )
)
...
(define-read-only (accumulative-funds-of (owner principal))
  (/
    (to-uint (+
        (to-int (* (var-get points-per-share) (ft-get-balance sp-001 owner)))
        (default-to 0 (map-get? points-correction owner))
    ))
    POINTS_MULTIPLIER
  )
...
(define-private (update-funds-balance (delta int))
  (begin
    ...
    (asserts! (and (> delta 0 ) (> (ft-get-supply sp-001) u0)) delta)
    ;; only distribute when funds are earned
    (var-set points-per-share (+ (var-get points-per-share) (/ (* (to-uint delta) POINTS_MULTIPLIER) (ft-get-supply sp-001))))
    delta
  )
)
...
(define-read-only (accumulative-losses-of (owner principal))
  (/
    (to-uint (+
      (to-int (* (var-get losses-per-share) (ft-get-balance sp-001 owner)))
      (default-to 0 (map-get? losses-correction owner))
    ))
    POINTS_MULTIPLIER
  )
...
(define-private (update-funds-lost (delta int))
  (begin
    ...
    (asserts! (and (> delta 0) (> (ft-get-supply sp-001) u0)) delta)
    ;; only distribute when funds are earned
    (var-set losses-per-share (+ (var-get losses-per-share) (/ (* (to-uint delta) POINTS_MULTIPLIER) (ft-get-supply sp-001))))
    delta
  )
)
