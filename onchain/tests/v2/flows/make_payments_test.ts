...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const amount_plus_rewards = (chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"]["ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5"]);
    const final_delegate_rewards = chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"]["ST3PF13W7Z0RRM42A8VZRVFQ75SV1K26RXEP8YGKJ"];
...
Clarinet.test({
  name: "Borrower completely pays their loan. Liquidity Providers (2) can claim all of their funds back + rewards.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST3NBRSFKX28FQ2ZJ1MAKX58HKHSDGNV5N7R21XCP
    ...
    block = pool.createPool(delegate_1.address,LP_TOKEN_0,ZP_TOKEN,PAYMENT,REWARDS_CALC,WITHDRAWAL_MANAGER,1000,1000,300_000_000_000,300_000_000_000,1,MAX_MATURITY_LENGTH,LIQUIDITY_VAULT,CP_TOKEN,COVER_VAULT,CP_REWARDS_TOKEN,XBTC,true);
    block = pool.finalizePool(delegate_1.address, LP_TOKEN_0, ZP_TOKEN, CP_TOKEN, poolId_0);
    ...
    assert(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][`${deployerWallet.address}.liquidity-vault-v1-0`] > minToReceive_1 + minToReceive_2);
    ...
    // console.log(WithdrawalManager.getNextExitCycle(chain, poolId_0, deployerWallet.address, "withdrawal-manager", deployerWallet.address));
    ...
    // // console.log(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"]["ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.liquidity-vault-v1-0"]);
    // // console.log(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"]["ST3PF13W7Z0RRM42A8VZRVFQ75SV1K26RXEP8YGKJ"]);
    // WithdrawalManager.getCycleShares(chain, poolId_0, 8, deployerWallet.address, "withdrawal-manager", deployerWallet.address).result.expectUint(requested_amount_1 + requested_amount_2);
    WithdrawalManager.getCycleShares(chain, poolId_0, 8, deployerWallet.address, "withdrawal-manager", deployerWallet.address).result.expectUint(requested_amount_1 + requested_amount_2);
    ...
    // WithdrawalManager.getCycleShares(chain, poolId_0, 5, deployerWallet.address, "withdrawal-manager", deployerWallet.address).receipts[0].result.expectUint(requested_amount_1);
    // WithdrawalManager.getCycleShares(chain, poolId_0, 5, deployerWallet.address, "withdrawal-manager", deployerWallet.address).receipts[0].result.expectUint(requested_amount_2);
    
    // assertEquals(chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][`${deployerWallet.address}.withdrawal-manager`], requested_amount);
    chain.mineEmptyBlock(14 * ONE_DAY * 2 - 1);
