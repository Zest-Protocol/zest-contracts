...
Clarinet.test({
  name: "Borrower makes 2 payments, misses 1. Borrower's loan gets liquidated with xBTC cover pool activated with cover and no collateral. 2 LPers redeem their funds with full losses.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG
    ...
    block = coverPool.sendFunds(CP_TOKEN,COVER_VAULT,CP_REWARDS_TOKEN,XBTC, poolId_0, 100_000_000_000n, 10, REWARDS_CALC, Cover_Provider.address);
