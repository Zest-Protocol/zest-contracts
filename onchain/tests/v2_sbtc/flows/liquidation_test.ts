...
Clarinet.test({
  name: "Borrower makes 2 payments, misses 1. Borrower's loan gets liquidated with xBTC cover pool activated with double cover and no collateral. 2 LPers redeem their funds without losses.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG
...
Clarinet.test({
  name: "Borrower makes 2 payments, misses 1. Borrower's loan gets liquidated with xBTC double cover and with half collateral. 2 LPers redeem their funds without losses.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG
    ...
    const supplierId = 0;
    const outputIndex = 0;
    const cover_amount = 200_000_000;

    let block = Globals.onboardUserAddress(chain, borrower_1.address, P2PKH_VERSION, HASH, deployerWallet.address);
    block = FT.transfer(chain,deployerWallet.address,"Wrapped-Bitcoin",1_000_000_000_000,deployerWallet.address,`${deployerWallet.address}.supplier-controller`,"",deployerWallet.address);
    block = FT.transfer(chain,deployerWallet.address,"Wrapped-Bitcoin",cover_amount,deployerWallet.address,Cover_Provider.address,"",deployerWallet.address);
    block = FT.transfer(chain,deployerWallet.address,"Wrapped-Bitcoin",40_000_000,deployerWallet.address,borrower_1.address,"",deployerWallet.address);
    ...
    block.receipts[0].result.expectOk().expectTuple()["cover-pool-recovered"].expectUint(loan_amount - coll_amount);
    ...
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][LP_1.address], sentAmount_1);
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][LP_2.address], sentAmount_2);
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][`${deployerWallet.address}.cover-vault`], cover_amount - (loan_amount - coll_amount));
  },
});

Clarinet.test({
  name: "Borrower makes 2 payments, misses 1. Borrower's loan gets liquidated with xBTC not enough cover or collateral. 2 LPers redeem their funds with losses.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG
    ...
    block = coverPool.sendFunds(CP_TOKEN,COVER_VAULT,CP_REWARDS_TOKEN,XBTC, poolId_0, 40_000_000n, 10, REWARDS_CALC, Cover_Provider.address);
    ...
    const used_cover = common.consumeUint(block.receipts[0].result.expectOk().expectTuple()["cover-pool-recovered"]);

    block.receipts[0].result.expectOk().expectTuple()["collateral-recovery"].expectUint(coll_amount);
    block.receipts[0].result.expectOk().expectTuple()["cover-pool-recovered"].expectUint(cover_amount);
    ...
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][LP_1.address], sentAmount_1 - ((loan_amount / 2) - ((coll_amount / 2) + (cover_amount / 2))));
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][LP_2.address], sentAmount_2 - ((loan_amount / 2) - ((coll_amount / 2) + (cover_amount / 2))));
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][`${deployerWallet.address}.cover-vault`], cover_amount - used_cover);
  },
});
...
Clarinet.test({
  name: "Borrower makes 2 payments, misses 1. Borrower's loan gets liquidated with enough collateral and cover. 2 LPers redeem their funds with no losses.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG
    ...
    block = FT.transfer(chain,deployerWallet.address,"Wrapped-Bitcoin",1_000_000_000_000,deployerWallet.address,`${deployerWallet.address}.supplier-controller`,"",deployerWallet.address);
    block = FT.transfer(chain,deployerWallet.address,"Wrapped-Bitcoin",cover_amount,deployerWallet.address,Cover_Provider.address,"",deployerWallet.address);
    block = FT.transfer(chain,deployerWallet.address,"Wrapped-Bitcoin",100_000_000,deployerWallet.address,borrower_1.address,"",deployerWallet.address);
    ...
    block.receipts[0].result.expectOk().expectTuple()["collateral-recovery"].expectUint(coll_amount);
    block.receipts[0].result.expectOk().expectTuple()["cover-pool-recovered"].expectUint(0);
    ...
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][LP_1.address], sentAmount_1);
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][LP_2.address], sentAmount_2);
    assertEquals(chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][`${deployerWallet.address}.cover-vault`], cover_amount);
  },
});
