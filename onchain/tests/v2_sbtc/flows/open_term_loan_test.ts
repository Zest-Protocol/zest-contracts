...
import { Pool } from '../interfaces/pool-v1-0.js';
import { CoverPool } from '../interfaces/cover-pool-v1-0.js';
import { Loan } from '../interfaces/loan-v1-0.js';
import { LPToken } from '../interfaces/lp-token.js';
import { CPToken } from '../interfaces/cp-token.js';
import { Buffer } from "https://deno.land/std@0.159.0/node/buffer.ts";
import { TestUtils } from '../interfaces/test-utils.js';
import { Magic } from '../interfaces/magic_real.js';
import { Globals } from '../interfaces/globals.js';
import { SupplierInterface } from '../interfaces/supplier_interface.js';
import { SwapRouter } from '../interfaces/swapRouter.js';
import { MagicCaller } from '../interfaces/magic-caller.js';
import { Payment } from '../interfaces/payment.js';
import { WithdrawalManager } from '../interfaces/withdrawal-manager.js';
import { CollVault } from '../interfaces/coll-vault.js';
import { FT } from '../interfaces/ft.js';
...
Clarinet.test({
  name: "Borrower initiates a loan. Delegate calls a loan (impairing a loan). Grace Period passes and loan gets liquidated, collateral is recovered. LP_1 and LP_2 can withdraw with losses.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    block = pool.createLoan(LOAN_V1_0,LP_TOKEN_0, poolId_0, loan_amount, XBTC, coll_ratio, XBTC, apr, 0, payment_period, COLL_VAULT, FUNDING_VAULT, borrower_1.address);
    loan.getLoanData(0)["remaining-payments"].expectUint(0);
    ...
    loan.getLoanData(0)["next-payment"].expectUint(0);
    ...
    loan.getLoanData(0)["next-payment"].expectUint((block.height - 2) + payment_period);
    loan.getLoanData(0)["remaining-payments"].expectUint(0);
    ...
    block = pool.callLoan(poolId_0, loanId, delegate_1.address);
    loan.getLoanData(0)["next-payment"].expectUint(block.height - 1);
    loan.getLoanData(0)["remaining-payments"].expectUint(1);
    chain.mineEmptyBlock(common.consumeUint(globals["grace-period"]) + 1);
...
Clarinet.test({
  name: "Borrower initiates a loan. Delegate calls a loan (impairing a loan). Borrower makes a full payment + principal. LP_1 and LP_2 can withdraw.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG
    ...
    const exitAtCycle =(common.consumeUint(WithdrawalManager.getNextExitCycle(chain, poolId_0, deployerWallet.address,"withdrawal-manager", deployerWallet.address).result));
    ...
    WithdrawalManager.getCycleShares(chain, poolId_0, exitAtCycle, deployerWallet.address, "withdrawal-manager", deployerWallet.address).result.expectUint(requested_amount_1 + requested_amount_2);
    WithdrawalManager.getCycleSharesByPrincipal(chain, poolId_0, LP_1.address, deployerWallet.address, "withdrawal-manager", deployerWallet.address).result.expectUint(requested_amount_1);
    WithdrawalManager.getCycleSharesByPrincipal(chain, poolId_0, LP_2.address, deployerWallet.address, "withdrawal-manager", deployerWallet.address).result.expectUint(requested_amount_2);
    ...
    assertEquals(chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][`${deployerWallet.address}.withdrawal-manager`], 0);
    assertEquals(chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][`${deployerWallet.address}.pool-v2-0`], 0);
    assertEquals(chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][`${LP_1.address}`], 0);
    assertEquals(chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][`${LP_2.address}`], 0);
    ...
    const total_rewards = Math.floor(payment_period * (apr * loan_amount) / 10000 / (DAYS_PER_YEAR * ONE_DAY));
...
Clarinet.test({
  name: "Borrower initiates a loan. Delegate calls a loan (impairing a loan). Borrower reverses call by rolling over the loan with different terms, but still open. Borrower completes payment. LP_1 and LP_2 can withdraw.",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    let deployerWallet = accounts.get("deployer") as Account;
    let LP_1 = accounts.get("wallet_1") as Account; // LP_1 ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5
    let LP_2 = accounts.get("wallet_2") as Account; // LP_2 ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG
    ...
    loan.getRolloverData(0).result.expectTuple()["maturity-length"].expectUint(0);

    block.receipts[0].result.expectOk();
    // console.log(loan.getRolloverData(0).result.expectTuple());
    // console.log(block.receipts[0].events[0].contract_event.value.expectTuple()["payload"].expectTuple()["data"].expectTuple());
