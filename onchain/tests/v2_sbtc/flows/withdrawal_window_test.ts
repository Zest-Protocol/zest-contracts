...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    let temp = chain.mineBlock([
      common.createMagicCallerDeployTx(
        MAGIC_CALLER_CONTRACT_NAME,
        LP_1.address
      ),
    ]);
    temp = chain.mineBlock([
      common.createMagicCallerDeployTx(
        MAGIC_CALLER_CONTRACT_NAME,
        borrower_1.address
      ),
    ...
    let block = Globals.onboardUserAddress(
      chain,
      borrower_1.address,
      P2PKH_VERSION,
      HASH,
    ...
    block = chain.mineBlock([
      ...
      SupplierInterface.updateLiquidity(
        chain.blockHeight,
        1_000_000_000_000,
        deployerWallet.address
      ),
    ...
    let MagicId_LP_1 = common.consumeUint(
      chain
        .callReadOnlyFn(
    ...
    let MagicId_Borrower = common.consumeUint(
      chain
        .callReadOnlyFn(
    ...
    let fee = Number(
      Magic.getSupplier(chain, 0, deployerWallet.address)
        .expectSome()
        .expectTuple()
        ["inbound-fee"].expectSome()
    );

    assert(
      chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][
    ...
    let swapId = common.consumeUint(
      block.receipts[0].result.expectOk().expectTuple()["swap-id"]
    );
    ...
    // mine dummy
    let globals = Globals.getGlobals(
      chain,
      deployerWallet.address
    ).expectTuple();
    const treasuryFeeBPs = common.consumeUint(globals["treasury-fee"]);
    const loanAmountWithoutTreasuryFee =
      loan_amount - (loan_amount * treasuryFeeBPs) / 10_000;
    let tx2 = util.generateP2PKHTx(HASH, loanAmountWithoutTreasuryFee);

    block = chain.mineBlock(
      common.finalizeDrawdown(
        tx2,
        chain.blockHeight - 1,
        loanId,
        LP_TOKEN_0,
    ...
    block.receipts[1].result
      .expectOk()
      .expectTuple()
      ["borrow-amount"].expectUint(loanAmountWithoutTreasuryFee);

    // Payment start
    chain.mineEmptyBlock(
      common.consumeUint(loan.getLoanData(0)["next-payment"]) - block.height - 1
    );

    const preimagePayment1 = "00";
    let hashPayment1 = util.getHash(preimagePayment1);
    let payment1 = common.consumeUint(
      Payment.getCurrentLoanPayment(chain, 0, borrower_1.address)
    );
    const payment1Tx = util.generateP2SHTx(
      sender,
      recipient,
      defaultExpiration,
      hashPayment1,
    ...
    assertEquals(
      chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][
        `${deployerWallet.address}.withdrawal-manager`
      ],
      requested_amount
    );
    ...
    const amount_plus_rewards =
      chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][
        LP_1.address
      ];
    const final_delegate_rewards =
      chain.getAssetsMaps().assets[".Wrapped-Bitcoin.wrapped-bitcoin"][
        delegate_1.address
      ];
    const total_rewards =
      num_payments *
      Math.floor(
        (payment_period * (apr * loan_amount)) /
          10000 /
          (DAYS_PER_YEAR * ONE_DAY)
      );

    assertEquals(
      amount_plus_rewards + final_delegate_rewards - sentAmount_1,
      total_rewards
    );
    assertEquals(
      chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][
        "ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.withdrawal-manager"
      ],
      0
    );
    assertEquals(
      chain.getAssetsMaps().assets[".lp-token-0.lp-token-0"][
        "ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.pool-v2-0"
      ],
      0
    );
    assertEquals(
