...
describe("Supply and Redeem", () => {
  beforeEach(() => {
    ...
    callResponse = simnet.callPublicFn("pool-reserve-data", "set-flashloan-fee-protocol", [ Cl.contractPrincipal(deployerAddress, stSTX), Cl.uint(35) ], deployerAddress);
  ...
  it("Supply and immediately redeem without returns", () => {
    ...
    expect(callResponse.result).toBeOk(Cl.uint(200_000_008 / 2));
    ...
    expect(callResponse.result).toBeOk(Cl.uint(100_000_005));
  ...
  it("Supply an asset that can't be used as collateral. Is not added to collateral in user value. Cannot be enabled as collateral or is not enabled when supplying again.", () => {
    ...
    expect(callResponse.result).toBeOk(Cl.uint(10_000_000));
