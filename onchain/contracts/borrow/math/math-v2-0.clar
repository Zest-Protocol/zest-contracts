...
(define-read-only (get-rt-by-block (rate uint) (blocks uint))
  (let (
    (start-time (unwrap-panic (get-stacks-block-info? time (- stacks-block-height blocks))))
    ;; (end-time (unwrap-panic (get-stacks-block-info? time (- stacks-block-height u2))))
  )
    ;; (/ (* rate (- end-time start-time)) seconds-in-block)
...
(define-read-only (get-block-info-0)
  (get-stacks-block-info? time stacks-block-height)
)

(define-read-only (get-block-info-1)
  (get-stacks-block-info? time (- stacks-block-height u1))
)

(define-read-only (get-block-info-2)
  (get-stacks-block-info? time (- stacks-block-height u2))
)

(define-read-only (test (height uint))
  (begin
    (ok {
