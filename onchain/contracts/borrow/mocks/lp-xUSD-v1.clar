...
(define-read-only (get-principal-balance (account principal))
  (ok 
    (+
      (unwrap-panic (contract-call? .lp-xusd get-principal-balance account))
...
;; (map-set approved-contracts .pool-borrow-v1-1 true)
;; (map-set approved-contracts .liquidation-manager-v1-1 true)
;; (map-set approved-contracts .pool-0-reserve true)
;; (map-set approved-contracts .borrow-helper true)
