...
(define-constant zsbtc .lp-sbtc-v3)
...
(define-read-only (get-sbtc-rewards (who principal))
    (let (
        ;; gets with interest
        (reward-balance (unwrap-panic (convert-to sbtc wstx (unwrap-panic (contract-call? .lp-sbtc-v3 get-balance who)))))
...
(define-read-only (get-user-program-index-eval
    ...
    (match (get-user-program-index who sbtc wstx)
        index (ok index)
        (let ((balance (unwrap-panic (contract-call? .lp-sbtc-v3 get-balance who))))
            (if (> balance u0)
