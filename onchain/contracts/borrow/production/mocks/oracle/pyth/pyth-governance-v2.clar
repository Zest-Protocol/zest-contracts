...
(define-private (parse-and-verify-ptgm (ptgm-bytes (buff 8192)) (sequence uint))
  (let ((cursor-magic (unwrap! (contract-call? .hk-cursor-v2 read-buff-4 { bytes: ptgm-bytes, pos: u0 }) 
          ERR_INVALID_PTGM))
        (cursor-module (unwrap! (contract-call? .hk-cursor-v2 read-buff-1 (get next cursor-magic)) 
          ERR_INVALID_PTGM))
        (cursor-action (unwrap! (contract-call? .hk-cursor-v2 read-buff-1 (get next cursor-module)) 
          ERR_INVALID_PTGM))
        (cursor-target-chain-id (unwrap! (contract-call? .hk-cursor-v2 read-buff-2 (get next cursor-action)) 
          ERR_INVALID_PTGM))
        (cursor-body (unwrap! (contract-call? .hk-cursor-v2 read-buff-8192-max (get next cursor-target-chain-id) none)
          ERR_INVALID_PTGM))
...
(define-private (parse-and-verify-fee-value (ptgm-body (buff 8192)))
  (let ((cursor-ptgm-body (contract-call? .hk-cursor-v2 new ptgm-body none))
        (cursor-mantissa (unwrap! (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-ptgm-body)) 
          ERR_INVALID_ACTION_PAYLOAD))
        (cursor-exponent (unwrap! (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-mantissa)) 
          ERR_INVALID_ACTION_PAYLOAD)))
...
(define-private (parse-and-verify-stale-price-threshold (ptgm-body (buff 8192)))
  (let ((cursor-ptgm-body (contract-call? .hk-cursor-v2 new ptgm-body none))
        (cursor-stale-price-threshold (unwrap! (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-ptgm-body)) 
          ERR_INVALID_ACTION_PAYLOAD)))
...
(define-private (parse-and-verify-governance-data-source (ptgm-body (buff 8192)))
  (let ((cursor-ptgm-body (contract-call? .hk-cursor-v2 new ptgm-body none))
        (cursor-emitter-chain (unwrap! (contract-call? .hk-cursor-v2 read-uint-16 (get next cursor-ptgm-body))
          ERR_INVALID_ACTION_PAYLOAD))
        (cursor-emitter-sequence (unwrap! (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-emitter-chain))
          ERR_INVALID_ACTION_PAYLOAD))
        (cursor-emitter-address (unwrap! (contract-call? .hk-cursor-v2 read-buff-32 (get next cursor-emitter-sequence))
          ERR_INVALID_ACTION_PAYLOAD)))
...
(define-private (parse-principal (ptgm-body (buff 8192)))
  (let ((cursor-ptgm-body (contract-call? .hk-cursor-v2 new ptgm-body none))
        (cursor-principal-len (try! (contract-call? .hk-cursor-v2 read-uint-8 (get next cursor-ptgm-body))))
        (principal-bytes (contract-call? .hk-cursor-v2 slice (get next cursor-principal-len) (some (get value cursor-principal-len))))
...
(define-private (parse-and-verify-prices-data-sources (ptgm-body (buff 8192)))
  (let ((cursor-ptgm-body (contract-call? .hk-cursor-v2 new ptgm-body none))
        (cursor-num-data-sources (try! (contract-call? .hk-cursor-v2 read-uint-8 (get next cursor-ptgm-body))))
        (cursor-data-sources-bytes (contract-call? .hk-cursor-v2 slice (get next cursor-num-data-sources) none))
...
(define-private (parse-data-source
  ...
  (if (is-eq (len (get result acc)) (get limit acc))
    ...
    (if (is-eq (get index (get cursor acc)) (get next-update-index (get cursor acc)))
      ;; Parse update
      (let ((buffer (contract-call? .hk-cursor-v2 new (get bytes acc) (some (get index (get cursor acc)))))
            (cursor-emitter-chain (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-16 (get next buffer))))
            (cursor-emitter-address (unwrap-panic (contract-call? .hk-cursor-v2 read-buff-32 (get next cursor-emitter-chain)))))
        {
          cursor: { 
