...
(define-public (commit-funds
  ...
  (begin
    ...
    (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.supplier-interface send-funds block prev-blocks tx proof output-index sender recipient expiration-buff hash swapper-buff supplier-id min-to-receive token-id loan-id action DEPLOYER))
  )
)
...
(define-public (send-funds-to-pool
  ...
  (let (
    ...
    (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.supplier-interface send-funds-to-pool txid lp l-v xbtc-ft r-c)
  )
)
