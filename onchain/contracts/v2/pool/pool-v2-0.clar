...
(define-public (send-funds (lp-token <sip-010>) (token-id uint) (asset <ft>) (assets uint) (recipient principal))
  (let (
    (shares (unwrap-panic (preview-funds-sent lp-token asset assets)))
    )
    (try! (contract-call? lp-token mint shares recipient))
    (ok true)
  )
)
...
(define-public (signal-redeem (lp-token <sip-010>) (token-id uint) (asset <ft>) (shares uint) (owner principal))
  (contract-call? .withdrawal-manager signal-redeem lp-token token-id asset shares owner)
  ;; (ok true)
)

;; -- View functions
;; TODO: test when having small amount of assets remaining
(define-public (convert-to-assets (lp-token <sip-010>) (asset <ft>) (shares uint))
  (let (
    (shares-supply (unwrap-panic (contract-call? lp-token get-total-supply)))
    (assets (if (is-eq shares-supply u0) shares (/ (* shares (unwrap-panic (total-assets lp-token asset))) shares-supply))))
    (ok assets)))

(define-public (convert-to-shares (lp-token <sip-010>) (asset <ft>) (assets uint))
  (let (
    (shares-supply (unwrap-panic (contract-call? lp-token get-total-supply))))
    (ok (/ (* shares-supply assets) (unwrap-panic (total-assets lp-token asset))))))

(define-public (convert-to-exit-shares (lp-token <sip-010>) (asset <ft>) (assets uint))
  (let (
    (shares-supply (unwrap-panic (contract-call? lp-token get-total-supply)))
    ;; TODO: take into unrealized losses
    (losses u0))
    (ok (/ (* shares-supply assets) (- (unwrap-panic (total-assets lp-token asset)) losses))))
)

(define-public (preview-funds-sent (lp-token <sip-010>) (asset <ft>) (assets uint))
  (convert-to-shares lp-token asset assets))

(define-public (preview-mint (lp-token <sip-010>) (asset <ft>) (shares uint))
  (let (
    (shares-supply (unwrap-panic (contract-call? lp-token get-total-supply)))
    (assets (if (is-eq shares-supply u0) shares (/ (* shares (unwrap-panic (total-assets lp-token asset))) shares-supply))))
    (ok assets)))
...
(define-public (total-assets (lp-token <sip-010>) (asset <ft>))
  (ok (+
    (unwrap-panic (contract-call? asset get-balance (contract-of lp-token)))
    u0 ;; assets under loan management
  )))
