...
(define-trait payment-trait
  (
    (make-next-payment (<ft> <lv> uint <cp-token> <swap> uint uint uint <ft> principal) (response { reward: uint, z-reward: uint, repayment: bool } uint))
    (make-full-payment (<ft> <lv> uint <cp-token> <swap> uint uint uint <ft> principal) (response { reward: uint, z-reward: uint, full-payment: uint } uint))
  )
)
