...
import { blockchainRequest, generateHTLCAddress, generateRandomHexString, sendToAddress, setupBitcoinAddresses, walletRequest, numberToLE, getHash, reverseBuffer, generateHTLCScript, getScriptHash, parseBtcAddress, getProof, waitForTxConfirmation, encodeExpiration } from './bitcoin';
import { setupContracts, makeContractCalls, broadcastTransactions, sendTransactionCalls, waitForStacksBlock, Loan, initializeSwapper, onboardUserAddress } from './stacks';
...
(async function() {
  ...
  const borrowerStxAddress = getStxAddress({ account: accounts.borrower.accounts[0], transactionVersion: TransactionVersion.Testnet })
  ...
  // initializing LP_1 as swapper to he Magic Protocol
  await initializeSwapper(lp1Account, network);

  let parsedBorrowerAddr = parseBtcAddress(borrower1Address as string);
  let broadcastResponse = await onboardUserAddress(accounts.deployer.accounts[0], borrowerStxAddress, numberToHex(parsedBorrowerAddr.version), parsedBorrowerAddr.hash.toString("hex"), network);
  ...
  let initNonce = ( await getNonce( getStxAddress({ account: accounts.deployer.accounts[0], transactionVersion: TransactionVersion.Testnet }), network) );
  ...
  let args = {
    ...
    supplierId: uintCV(0),
    minToReceive: uintCV((10000000 * 10) / 10_000),
  };
  ...
  let transactions = [{
    ...
    contractName: BridgeContract.name,
    functionName: BridgeContract.Functions.InitializeSwapper.name,
    functionArgs: [],
  ...
  initNonce = await getNonce(getStxAddress({ account: borrower1Account, transactionVersion: TransactionVersion.Testnet }), network);
  ...
  broadcastResponse = await broadcastTransaction(transaction, network);
  debug(JSON.stringify(broadcastResponse));
  console.log('Claim Rewards finalized.');
})();
