...
import { networks, payments, Psbt, address, script as bScript, opcodes } from 'bitcoinjs-lib';
...
(async function() {
  ...
  const supplierAddress = payments.p2pkh({ pubkey: supplierBtcSigner.publicKey, network }).address!;
  ...
  let preimage1 = Buffer.from(generateRandomHexString(32), "hex");
  ...
  psbt.signInput(0, supplierBtcSigner);
